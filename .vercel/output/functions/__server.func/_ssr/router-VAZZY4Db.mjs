...
import { $t as ATTR_OPERATION_ID, A as base64Url, At as any, Bt as record, C as runWithEndpointContext, Cn as BetterAuthError, D as serializeSignedCookie, E as serializeCookie, En as defineErrorCodes, Ft as literal, Ht as union, It as looseObject, Lt as number, M as decodeProtectedHeader, Mt as boolean, N as importJWK, Nt as discriminatedUnion, O as filterOutputFields, P as jwtVerify, Pt as email, Qt as ATTR_HOOK_TYPE, Rt as object, S as getCurrentAuthContext, Sn as APIError, T as toResponse, Tn as BASE_ERROR_CODES, Ut as unknown, Vt as string, Xt as withSpan, Zt as ATTR_CONTEXT, _ as createAuthMiddleware, a as betterFetch, an as runWithAdapter, b as hasRequestState, bn as isProduction, c as applyDefaultAccessTokenExpiry, cn as initGetModelName, d as findInvalidTrustedProxies, dn as createRandomStringGenerator, en as import_src, f as getIp, fn as createLogger, g as createAuthEndpoint, gn as env, h as normalizePathname, ht as JWTExpired, i as refreshAccessToken, in as queueAfterTransactionHook, j as decodeJwt, jt as array, kt as _enum, l as isLoopbackHost, ln as initGetFieldName, mn as shouldPublishLog, n as socialProviders, nn as getAuthTables, on as runWithTransaction, p as deprecate, pn as logger, r as validateAuthorizationCode, rn as getCurrentAdapter, rt as encode, s as createAuthorizationURL, sn as getBetterAuthVersion, t as SocialProviderListEnum, tn as safeJSONParse, u as createRateLimitKey, un as generateId, v as isAPIError, w as createRouter$1, wn as kAPIErrorHeaderSymbol, x as runWithRequestState, xn as isTest, y as defineRequestState, yn as isDevelopment, zt as optional } from "../_libs/@better-auth/core+[...].mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/router-VAZZY4Db.js
...
var styles_default = "/assets/styles-uhmF0PP5.css";
var APP_NAME = "FutureLife";
var Route$4 = createRootRoute({
	head: () => ({
		meta: [
			...
			{
				name: "description",
				content: "Fly a living voxel sky of floating isles, lake-bowls, and giant canopy trees."
			}
		],
		links: [
			{
...
var $$splitComponentImporter$1 = () => import("./routes-DiWBtRiJ.mjs").then((n) => n.t);
var Route$3 = createFileRoute("/")({ component: lazyRouteComponent($$splitComponentImporter$1, "component") });
var $$splitComponentImporter = () => import("./login-DaMMOo6L.mjs");
...
function ensureSchema(sql) {
	globalRef.__rtcSchemaPromise__ ??= (async () => {
		...
		await sql.query(`ALTER TABLE webrtc_peers ADD COLUMN IF NOT EXISTS spawn_idx INTEGER NOT NULL DEFAULT 0`);
		await sql.query(`CREATE TABLE IF NOT EXISTS webrtc_room_seq (
...
async function claimIdentity(sql, room, peer, requested) {
	const want = sanitizeName(requested);
	const custom = isCustomName(want);
	const row = (await sql.query(`SELECT name, spawn_idx FROM webrtc_peers WHERE room = $1 AND peer_id = $2`, [room, peer]))[0];
	if (row) {
		const name = custom ? want : row.name;
		await sql.query(`UPDATE webrtc_peers SET last_seen = now(), name = $3 WHERE room = $1 AND peer_id = $2`, [
			room,
			peer,
			name
		]);
	...
	await sql.query(`INSERT INTO webrtc_room_seq (room, next_guest, next_spawn)
     VALUES ($1, 1, 0)
     ON CONFLICT (room) DO NOTHING`, [room]);
	const seq = (await sql.query(`UPDATE webrtc_room_seq
     SET next_guest = next_guest + $2,
         next_spawn = next_spawn + 1
     WHERE room = $1
     RETURNING next_guest - $2 AS guest_n, next_spawn - 1 AS spawn`, [room, custom ? 0 : 1]))[0] ?? {
		guest_n: 1,
		spawn: 0
	};
	const name = custom ? want : `Guest ${seq.guest_n}`;
	await sql.query(`INSERT INTO webrtc_peers (room, peer_id, name, spawn_idx, last_seen)
     VALUES ($1, $2, $3, $4, now())
